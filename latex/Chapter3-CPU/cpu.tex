\chapter{The Beta machine CPU and IO unit design}
% PF Maybe use $\beta$ machine?  I think it looks better
% QP I prefer keeping "beta" because I don't want to mess up with my figures and 
% stuff due to a ctrl-f > replace all
...
\href{https://github.com/AnotherGitAccount/TFE/blob/master/hardware/rtl/cpu}{Github link to the verilog files of the CPU.}
...
%PF There are ways to do it in a less expensive way, with a little logic arround pcsel and only one of these multiplexers
% QP OK discussed by mail
...
%PF allows: si en français, on utilise beaucoup ce genre de construction
% en Anglais, on l'évite plutôt
% QP OK
...
\begin{table}[H]
    ...
    % PF Add punctuation at the end of captions.  See Chicago Manual of Style, 16th ed, page 122 (download from libgen).  This is a reference to know how to type anything in scientific literature.
    \label{tab:alu/sim}
\end{table}
...
Now that the module based on the altsyncram is created,  an interface to enable 
...
% PF This aboves surprises me.  Maybe when you have time, we can discuss this.
% QP OK
...
% PF I do not understand why this last sentence is there.  Maybe better link?
% QP yes, it was an error
...
% PF must be able to be used as: this is heavy, try to rephrase the whole sentence in a nicer way
% QP OK
...
Only one signal, pcsel, must be added to the control logic: it will allow the program 
...
\href{https://github.com/AnotherGitAccount/TFE/blob/master/hardware/rtl/iou}{Github link to the verilog files of the IOU.}
...
When it comes to reads and writes, the data word contains the value of all the elements of the addressed 
